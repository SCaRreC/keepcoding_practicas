...
10. ¿Que punto de corte sería el aconsejable?, ¿cuantos clusters aparecen?
    ...
    # Los valores de silhouette son cercanos a 1 por lo que indica un buen grupo de cluster, aunque en algunos casos, algunos indices son negativos.
    ```
...
13. Tratamos de predecir los metros cuadrados en función del resto de columnas del dataframe.
    ...
    ```{r}
        # Aplicando regresión lineal sobre las variables que considero importantes, sin estandarizar y sin seleccionarlas basadas en ninguna método analítico.
    ...
    model <- lm(data=madrid.train, formula = Square.Meters~Accommodates+Bathrooms+Bedrooms+Beds+Price+Guests.Included+Extra.People+Review.Scores.Rating+neighb_id)
        ...
        # Si uso la variable de predicción "Scores.Rating", hay varios datos cuyo valor es NA, por lo que el modelo lineal no puede estimar los metros cuadrados. Una opcion es eliminar esta variable como predictora.
...
# El modelo tiene un error y R-sq que se podrían mejorar, pero para el conjunto testing, vemos que los valores son mejores, asi que podría valer este modelo.
```
...
hist(model$residual)
# Residuos distribuidos más o menos normalmente, pero con algunos outliers. Vamos a ver que pasaría si eliminamos esos outliers y si el modelo mejoraría.
```

```{r}
    plot(cooks.distance(model)) # se podrian eliminar los outliers que aparecen.

    madrid.train_clean <- madrid.train[cooks.distance(model) < 0.1, ]
    abline(h = 0.1, col = "red", lty = 2, lwd = 2)

    model_madrid_clean<-lm(data=madrid.train_clean, formula = Square.Meters~Accommodates+Bathrooms+Bedrooms+Beds+Price+Guests.Included+Extra.People+Review.Scores.Rating+neighb_id)
    summary(model_madrid_clean)
    
   dim(madrid.train[cooks.distance(model) > 0.1, ])
```
```{r}

madrid.test <- df_madrid[-train_ind,]

madrid.test$sqm_est <- predict(model_madrid_clean, madrid.test)
madrid.train_clean$sqm_est <- predict(model_madrid_clean, madrid.train_clean)
df_sqm <- data.frame(merge(madrid.test$Square.Meters,madrid.test$sqm_est ))
df_sqm <- data.frame(merge(madrid.train_clean$Square.Meters,madrid.train_clean$sqm_est ))

print("Training:")
postResample(madrid.train_clean$sqm_est,obs = madrid.train_clean$Square.Meters)
print("Testing:")
postResample(madrid.test$sqm_est,obs = madrid.test$Square.Meters)
```
...
# Disminuye el Residual standard error y empeora la R-squared después de filtrar en los datos de entrenamiento los outliers, pero, tras comprobar, de qué datos se trata, resulta que son la mayoría de los pisos con "neighd_id = 3", así que continuo con el primer modelo creado para tener mejor representación de todos los grupos de "neighb_id" en el conjunto de training.
```
...
df_sqm_est <- predict(model, df_madrid)
df_madrid <- df_madrid |> mutate(Square.Meters = coalesce(Square.Meters, df_sqm_est))
